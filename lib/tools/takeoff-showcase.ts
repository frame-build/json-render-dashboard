...
import type { ShowcaseContextData } from "@/lib/chat/types";

export function getTakeoffShowcasePayload(
  focus?: string | null,
): ShowcaseContextData | { error: string } {
    ...
    supportedDashboards: [
      {
        mode: "Walls",
        emphasis:
          "Wall quantities, type filters, material filters, and level breakdowns",
      },
      {
        mode: "Floors",
...
export const getTakeoffShowcaseData = tool({
  ...
  execute: async ({ focus }) => {
    return getTakeoffShowcasePayload(focus);
  },
});
