...
import { z } from "zod";
import type { PromptRefinementOption } from "@/lib/chat/types";
...
export const explorerCatalog = defineCatalog(schema, {
  components: {
    ...
    ShowcaseDashboardLayout: {
      ...
      slots: ["default"],
      description:
        "Fixed APS showcase dashboard shell. Use this as the root for every dashboard. Children must appear in this exact order: 1) filters section, 2) KPI strip, 3) viewer panel, 4) primary analytics panel, 5) secondary analytics panel, 6) detail panel A, 7) detail panel B.",
      example: {
        title: "Wall estimating dashboard",
        description: "Viewer-first quantity dashboard for the showcase model.",
      },
    },
    ...
    ShowcaseKpiGrid: {
      ...
      slots: ["default"],
      description:
        "Deterministic KPI grid for showcase dashboards. Use this for the KPI strip section.",
      example: {
        columns: 2,
        gap: "md",
      },
    ...
    ShowcaseFilterGrid: {
      ...
      slots: ["default"],
      description:
        "Deterministic filter grid for showcase dashboards. Use this to organize slicers in the top filters section.",
      example: {
        columns: 2,
        gap: "md",
      },
    ...
    PromptRefinementChooser: {
      props: z.object({
        ...
        options: z.custom<PromptRefinementOption[]>(
          ...
          "Expected a list of prompt refinement options",
        ),
      }),
      description:
        ...
        example: {
          ...
          options: [
          {
            ...
            rationale:
              "Uses the exact Walls category with supported filters and quantities.",
          },
        ],
      },
    },
    ...
    ShowcasePaginatedTable: {
      props: z.object({
        ...
        pageSize: z.number().int().min(1).nullable(),
        emptyMessage: z.string().nullable(),
      }),
      description:
        'Showcase detail table with built-in pagination. Use { "$state": "/path" } on data and { "$bindState": "/ui/detail/<section>/page" } on page.',
      example: {
        title: "Detailed Breakdown",
        description: "Complete filtered schedule with pagination.",
    ...
    Tabs: {
      props: z.object({
        ...
        editorSectionKey: z.string().nullable(),
        tabs: z.array(
          z.object({
            value: z.string(),
            label: z.string(),
      ...
      slots: ["default"],
      description:
        'Tabbed content container. Use { "$bindState": "/path" } on value to keep tab selection in state.',
    },
    ...
    Pagination: {
      ...
      description:
        'Pagination control. Use { "$bindState": "/path" } on page so navigation state is shared across components.',
    },
    ...
    AutodeskViewer: {
      props: z.object({
        ...
        theme: z.enum(["light-theme", "dark-theme"]).nullable(),
        showModelBrowser: z.boolean().nullable(),
        fitToView: z.boolean().nullable(),
        isolatedDbIds: z.array(z.number()).nullable(),
        selectedDbIds: z.array(z.number()).nullable(),
        fitToSelection: z.boolean().nullable(),
      }),
      description:
        "Embedded Autodesk Platform Services viewer for real BIM/CAD models. Use this for takeoff, estimating, construction, or BIM showcase experiences with a valid translated APS URN.",
      example: {
        urn: { $state: "/showcase/model/urn" },
        height: "560px",
        theme: "dark-theme",
        showModelBrowser: false,
        fitToView: true,
        isolatedDbIds: { $state: "/analysis/viewer/isolatedDbIds" },
        selectedDbIds: null,
